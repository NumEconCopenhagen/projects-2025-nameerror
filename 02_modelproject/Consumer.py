...
class ConsumerClass:
    ...
    def ces(self,z1,z2,w,sigma):
        """ the CES aggregate of two inputs - is used both for the bundre and the combined utility
    ...
    def utility(self,x1,x2,x3):
        ...
        # here, we define utility, using the ces equation defined earlier.
        xb = self.ces(x2, x3, par.beta, par.sigma_B) # Define the travel bundle, with the weight on bus vs. train and
    ...
    def shares(self,s1,w):
        ...
        Args:
            ...
            w (float or ndarray): share of the travel budget spent on the bus and transport vs. food
    ...
    def value_of_choice(self,s1,w):
        ...
        #Insert the inside of equation 4:
    ...
    def solve_grid(self,N=200,do_print=True):
        """ solve by a 2-dimensional grid search (2.1.1) over the nested shares
        ...
        s1_grid,w_grid = np.meshgrid(s1_vec,w_vec,indexing='ij') #and the a shared grid

        # b. utility in every grid point
        u_grid=self.value_of_choice(s1_grid, w_grid)

        # c. the best point
        i_best= np.argmax(u_grid) 
    ...
    def solve(self,s0=None,do_print=True,**kwargs):
        ...
        res=optimize.minimize(self.objective, s0, method='L-BFGS-B', bounds=((0,1), (0,1)), 
                                    callback= lambda sk: path.append(sk.copy()), **kwargs) #So use the bounds and make sure to populate the path
